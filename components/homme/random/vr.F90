...
module vertremap_base
  ...
  integer, parameter :: real_kind = 8, r8 = real_kind, int_kind = 4
...
contains
  ...
  subroutine remap1(Qdp,nx,qsize,nlev,vert_remap_q_alg,dp1,dp2)
    ! remap 1 field
    ...
    !
    implicit none
    integer, intent(in) :: nx,qsize,nlev,vert_remap_q_alg
    ...
    do q=1,qsize
       ...
       enddo
    enddo ! q loop
    if (abort) print *,'Bad levels in remap1.  usually CFL violatioin'

  end subroutine remap1

  subroutine remap1_nofilter(Qdp,nx,qsize,nlev,dp1,dp2)
    ! remap 1 field
    ! input:  Qdp   field to be remapped (NOTE: MASS, not MIXING RATIO)
    !         dp1   layer thickness (source)
    !         dp2   layer thickness (target)
    ...
    do q=1,qsize
       ...
       enddo
    enddo ! q loop
    if (abort) print *,'Bad levels in remap1_nofilter.  usually CFL violatioin'
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine remap_Q_ppm(Qdp,nx,qsize,nlev,vert_remap_q_alg,dp1,dp2)
    ! remap 1 field
    ! input:  Qdp   field to be remapped (NOTE: MASS, not MIXING RATIO)
    !         dp1   layer thickness (source)
    !         dp2   layer thickness (target)
       ...
       do i = 1 , nx
          ...
          do k = 1 , nlev
             ...
             if (pio(kk) <= pin(k+1)) then
                ...
                call binary_search(pio, pin(k+1), kk, nlev)
             end if
  ...
  ! value for k.
  subroutine binary_search(pio, pivot, k, nlev)
  ...
  subroutine pl_remap1(Qdp,nx,qstart,qstop,qsize,nlev,vert_remap_q_alg,dp1,dp2)
    ! remap 1 field
    ! input:  Qdp   field to be remapped (NOTE: MASS, not MIXING RATIO)
    !         dp1   layer thickness (source)
    !         dp2   layer thickness (target)
    ...
    !
    implicit none
    integer, intent(in) :: nx,qstart,qstop,qsize,nlev,vert_remap_q_alg
    ...
    if (vert_remap_q_alg == 1 .or. vert_remap_q_alg == 2) then
       ...
       !  !$OMP END PARALLEL
       !else
       call pl_remap_Q_ppm(qdp,nx,qstart,qstop,qsize,nlev,vert_remap_q_alg,dp1,dp2)
       !endif
       !call t_stopf('remap_Q_ppm')
       return
    endif
  end subroutine pl_remap1

  subroutine pl_remap1_nofilter(Qdp,nx,qsize,nlev,vert_remap_q_alg,dp1,dp2)
    ! remap 1 field
    ! input:  Qdp   field to be remapped (NOTE: MASS, not MIXING RATIO)
    !         dp1   layer thickness (source)
    !         dp2   layer thickness (target)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    enddo ! q loop
    if (abort) then
       print *, 'Bad levels in remap1_nofilter.  usually CFL violatioin'
    end if
  end subroutine pl_remap1_nofilter
  ...
  !This uses the exact same model and reference grids and data as remap_Q, but it interpolates
  !using PPM instead of splines.
  subroutine pl_remap_Q_ppm(Qdp,nx,qstart,qstop,qsize,nlev,vert_remap_q_alg,dp1,dp2)
    ! remap 1 field
    ! input:  Qdp   field to be remapped (NOTE: MASS, not MIXING RATIO)
    !         dp1   layer thickness (source)
    !         dp2   layer thickness (target)
    ...
    do j = 1 , nx
       do i = 1 , nx
          ...
          !This turned out a big optimization, remembering that only parts of the PPM algorithm depends on the data, namely the
          !limiting. So anything that depends only on the grid is pre-computed outside the tracer loop.
          ppmdx(:,:) = pl_compute_ppm_grids( dpo,nlev,vert_remap_q_alg )
          ...
          do q = qstart, qstop
             ...
             do k = 1 , nlev
                kk = kid(k)
                massn2 = masso(kk) + pl_integrate_parabola( coefs(:,kk) , z1(k) , z2(k) ) * dpo(kk)
                Qdp(i,j,k,q) = massn2 - massn1
                massn1 = massn2
             enddo
          enddo
    ...
    enddo
    ! call t_stopf('remap_Q_ppm')
  end subroutine pl_remap_Q_ppm

  !=======================================================================================================!


  !THis compute grid-based coefficients from Collela & Woodward 1984.
  function pl_compute_ppm_grids( dx,nlev,vert_remap_q_alg )   result(rslt)
    ...
    enddo
  end function pl_compute_ppm_grids
  ...
  !This computes a limited parabolic interpolant using a net 5-cell stencil, but the stages of computation are broken up into 3 stages
  function pl_compute_ppm( a , dx,nlev,vert_remap_q_alg )    result(coefs)
  ...
  !Simple function computes the definite integral of a parabola in normalized coordinates, xi=(x-x0)/dx,
  !given two bounds. Make sure this gets inlined during compilation.
  function pl_integrate_parabola( a , x1 , x2 )    result(mass)
    implicit none
  ...
  end function pl_integrate_parabola
    ...
    do pl = 0,1
       do algidx = 1, size(algs)
          ...
          do i = 3,nme
             ...
             p1(1) = 0
             do k = 2,nlev+1
                p1(k) = sin(c1*pi*real(k,real_kind)/(nlev+1))/sin(c1*pi)
                dp1(k-1) = p1(k) - p1(k-1)
             end do
             ...
             do k = 1,nlev
                Qdp(1,1,k,1) = dp1(k)*eval_f(p1(k) + 0.5d0*dp1(k))
             end do
             ...
             ! Point evaluation rather than integrating over the cell
             ! limits OOA to 2.
             num = 0; den = 0
             do k = 1,nlev
                ft = eval_f(p2(k) + 0.5d0*dp2(k))
                f = Qdp(1,1,k,1)/dp2(k)
                num = num + abs(f - ft)**2
                den = den + ft**2
             end do
             ...
             if (first) then
                ...
                ooa = -(log(re) - log(re_prev))/log(2.0d0)
             end if
             write(*,'(a,i2,es12.2,f6.2)') 're> ',i,re,ooa
             re_prev = re
             first = .false.
          end do
       end do
  ...
  subroutine remap1c(nlev, alg, dp1i, dp2i, Qdpi) bind(c)
    use iso_c_binding

    integer(kind=c_int), value, intent(in) :: nlev, alg
    real(kind=c_double), intent(in) :: dp1i(nlev), dp2i(nlev)
    real(kind=c_double), intent(inout) :: Qdpi(nlev)

    real(kind=real_kind) :: dp1(1,1,nlev), dp2(1,1,nlev), Qdp(1,1,nlev,1)

    dp1(1,1,:nlev) = dp1i(:nlev)
...
program main
  ! standalone conv tester: gfortran vr.F90; if [ $? == 0 ]; then ./a.out; fi
  ! shared lib for use in python: gfortran -c -fPIC vr.F90; gfortran -shared vr.o -o libvr.so

  use vertremap_base, only: test
