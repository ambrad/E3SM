...
# ifdef HOMMEXX_BFB_TESTING
                ...
                call tridiag_diagdom_bfb_a1x1(nlev, JacLt(:,i,j), jacDt(:,i,j), jacUt(:,i,j), xt(:,i,j),INT(SIZEOF(JacL)/SIZE(JacL),4))
# else
#  ifdef NEWTONCOND
          ...
          if (maxval(dphi(:,:,1:nlev)) >= 0) then
             do j=1,np
                do i=1,np
                   ...
                   if (verbosity > 0) write(iulog,*) 'WARNING:IMEX is reducing step length from 1 to',alpha
