...
module prim_driver_base
  ...
  private
  public :: prim_init1, prim_init2 , prim_run_subcycle, prim_finalize, prim_run_subcycle_amb
...
!=======================================================================================================!
  ...
  subroutine prim_step_amb(elem, hybrid,nets,nete, dt, tl, hvcoord, compute_diagnostics)
    ...
    do ie=nets,nete
       ...
       elem(ie)%derived%dp(:,:,:)=elem(ie)%state%dp3d(:,:,:,tl%n0)
       elem(ie)%derived%eta_dot_dpdn=0     ! mean vertical mass flux
       call flt_start_new_interval(elem, tl)
    enddo
    ...
    do n=1,qsplit
       ...
       if (rsplit == 0) then
          call flt_update(elem, tl, dt, rsplit)
       else
          if (modulo(n, rsplit) == 0) then
             if (prescribed_wind == 1) then
                ...
                call flt_update(elem, tl, dt_remap, rsplit)
